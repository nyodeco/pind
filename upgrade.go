...
// oldPindHomeDir returns the OS specific home directory pind used prior to
// version 0.3.3.  This has since been replaced with pinutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldPindHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "pind")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".pind")
	}
...
// location prior to pind version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to pind
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldPindHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "pind.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "pind_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "pind_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to pind
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		pindLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old pind.conf into new location if needed.
		...
		if ohpEmpty {
			...
			pindLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to pind as new versions require it.
