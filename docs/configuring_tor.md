...
pind provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, pind supports a hybrid,
...
Configuring pind as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the pind command
line or in the pind configuration file.  Typically the Tor proxy address will be
...
By default, pind assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since pind will act as the server.
...
```text
HiddenServiceDir /var/tor/pind
HiddenServicePort 8333 127.0.0.1:8333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring pind as a Tor hidden service requires
three flags:
...
```bash
./pind --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
pind provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the pind command line or in the pind
...
```bash
./pind --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
pind provides support for Tor stream isolation by using the `--torisolation`
flag.  This option requires --proxy or --onionproxy to be set.

### Command line example

```bash
./pind --proxy=127.0.0.1:9050 --torisolation
```
