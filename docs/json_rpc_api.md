...
pind provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between pind and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, pind intentionally splits the wallet and chain
  services into independent processes.  See the blog post
  [here](https://blog.conformal.com/pind-not-your-moms-bitcoin-daemon/) for
  further details on why they were separated.  This means that if you are
  talking directly to pind, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [pinwallet](https://github.com/btcsuite/pinwallet).
* pind is secure by default which means that the RPC connection is TLS-enabled
  by default
* pind provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for pind RPC and are used by applications
such as [pinwallet](https://github.com/btcsuite/pinwallet) for inter-process
communication with pind.  The websocket connection endpoint for pind is
...
during the development of pinwallet.
...
The pind RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
pind is `wss://your_ip_or_domain:8334/ws`.
...
to a pind RPC server:

* **rpcuser** is the full-access username configured for the pind RPC server
* **rpcpass** is the full-access password configured for the pind RPC server
* **rpclimituser** is the limited username configured for the pind RPC server
* **rpclimitpass** is the limited password configured for the pind RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the pind
  server is configured with.  It is automatically generated by pind and placed
  in the pind home directory (which is typically `%LOCALAPPDATA%\Pind` on
  Windows and `~/.pind` on POSIX-like OSes)

**NOTE:** As mentioned above, pind is secure by default which means the RPC
...
The pind RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
pind comes with a separate utility named `pinctl` which can be used to issue
these RPC commands via HTTP POST requests to pind after configuring it with the
...
|25|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">pind does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|26|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since pind does not have the wallet integrated to provide payment addresses, pind must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|27|[stop](#stop)|N|Shutdown pind.|
|28|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|29|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since pind does not have a wallet integrated, pind will only return whether the address is valid or not.|
...
|Notes|NOTE: Since pind does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in pinwallet for a version which includes that information.|
...
|Notes|<font color="orange">Since pind does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Since pind does not have the wallet integrated to provide payment addresses, pind must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">pind does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown pind.|
|Returns|`"pind stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For pind this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">Pind currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for pind.</font>|
...
The following is an overview of the RPC methods which are implemented by pind, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network pind is running on.|None|
...
|Parameters|None|
|Description|Get bitcoin network pind is running on.|
|Returns|numeric|
...
|Method|version|
|Parameters|None|
|Description|Returns the version of the JSON-RPC API built into this release of pind.|
|Returns|`{ (json object)`<br />&nbsp;&nbsp;`"pindjsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "x.y.z",  (string) the version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": x,  (numeric) the major version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": y,  (numeric) the minor version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": z,  (numeric) the patch version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",  (string) prerelease info for the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""  (string) metadata about the server build`<br />&nbsp;&nbsp;`}`<br />`}`|
|Example Return|`{`<br />&nbsp;&nbsp;`"pindjsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "1.0.0",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": 1,  `<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""`<br />&nbsp;&nbsp;`}`<br />`}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this pind instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
pind uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll pind for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[rpcclient](https://github.com/nyodeco/pind/tree/master/rpcclient) package.
...
[rpcclient](https://github.com/nyodeco/pind/tree/master/rpcclient) package to connect with
a pind instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by pind when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[rpcclient](https://github.com/nyodeco/pind/tree/master/rpcclient) package to connect with
a pind instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/nyodeco/pind/tree/master/rpcclient) package to connect with
a pind instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a pind instance,
...
// generated by pind when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/pind/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The pind generated certificate acts as
