...
func TestWalletSvrWsNtfns(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "accountbalance",
			newNtfn: func() (interface{}, error) {
				return pinjson.NewCmd("accountbalance", "acct", 1.25, true)
			},
			staticNtfn: func() interface{} {
				return pinjson.NewAccountBalanceNtfn("acct", 1.25, true)
			},
			marshalled: `{"jsonrpc":"1.0","method":"accountbalance","params":["acct",1.25,true],"id":null}`,
			unmarshalled: &pinjson.AccountBalanceNtfn{
				Account:   "acct",
				Balance:   1.25,
				Confirmed: true,
			},
		},
		{
			name: "pindconnected",
			newNtfn: func() (interface{}, error) {
				return pinjson.NewCmd("pindconnected", true)
			},
			staticNtfn: func() interface{} {
				return pinjson.NewPindConnectedNtfn(true)
			},
			marshalled: `{"jsonrpc":"1.0","method":"pindconnected","params":[true],"id":null}`,
			unmarshalled: &pinjson.PindConnectedNtfn{
				Connected: true,
			},
		},
		{
			...
			newNtfn: func() (interface{}, error) {
				return pinjson.NewCmd("walletlockstate", true)
			},
			staticNtfn: func() interface{} {
				return pinjson.NewWalletLockStateNtfn(true)
			},
			marshalled: `{"jsonrpc":"1.0","method":"walletlockstate","params":[true],"id":null}`,
			unmarshalled: &pinjson.WalletLockStateNtfn{
				Locked: true,
			},
		},
		{
			name: "newtx",
			newNtfn: func() (interface{}, error) {
				return pinjson.NewCmd("newtx", "acct", `{"account":"acct","address":"1Address","category":"send","amount":1.5,"bip125-replaceable":"unknown","fee":0.0001,"confirmations":1,"trusted":true,"txid":"456","walletconflicts":[],"time":12345678,"timereceived":12345876,"vout":789,"otheraccount":"otheracct"}`)
			},
			staticNtfn: func() interface{} {
				result := pinjson.ListTransactionsResult{
					Abandoned:         false,
					Account:           "acct",
					Address:           "1Address",
				...
				return pinjson.NewNewTxNtfn("acct", result)
			},
			...
			unmarshalled: &pinjson.NewTxNtfn{
				Account: "acct",
				Details: pinjson.ListTransactionsResult{
					Abandoned:         false,
					Account:           "acct",
					Address:           "1Address",
					BIP125Replaceable: "unknown",
