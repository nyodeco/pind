...
func TestAssignField(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := pinjson.TstAssignField(1, "testField", dst, src)
		if err != nil {
...
func TestNewCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			method: "boguscommand",
			args:   []interface{}{},
			err:    pinjson.Error{ErrorCode: pinjson.ErrUnregisteredMethod},
		},
		{
			name:   "too few parameters to command with required + optional",
			method: "getblock",
			args:   []interface{}{},
			err:    pinjson.Error{ErrorCode: pinjson.ErrNumParams},
		},
		{
...
func TestMarshalCmd(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		bytes, err := pinjson.MarshalCmd(pinjson.RpcVersion1, test.id, test.cmd)
		if err != nil {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v)",
				i, test.name, err, err)
			continue
...
func TestMarshalCmdErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			id:   1,
			cmd:  (*pinjson.GetBlockCmd)(nil),
			err:  pinjson.Error{ErrorCode: pinjson.ErrInvalidType},
		},
		{
			name: "nil instance of registered type",
			id:   []int{0, 1},
			cmd:  &pinjson.GetBlockCountCmd{},
			err:  pinjson.Error{ErrorCode: pinjson.ErrInvalidType},
		},
	}
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := pinjson.MarshalCmd(pinjson.RpcVersion1, test.id, test.cmd)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v), "+
				"want %T", i, test.name, err, err, test.err)
			continue
...
func TestUnmarshalCmdErrors(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := pinjson.UnmarshalCmd(&test.request)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v), "+
				"want %T", i, test.name, err, err, test.err)
			continue
