...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if pinjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *pinjson.RPCError {
				return pinjson.NewRPCError(pinjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := pinjson.MarshalResponse(pinjson.RpcVersion1, testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := pinjson.NewRequest(pinjson.RpcVersion1, nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := pinjson.Error{ErrorCode: pinjson.ErrInvalidType}
	_, err = pinjson.MarshalResponse(pinjson.RpcVersion1, make(chan int), nil, nil)
	if jerr, ok := err.(pinjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = pinjson.MarshalResponse(pinjson.RpcVersion1, 1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			pinjson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			pinjson.ErrRPCMethodNotFound,
