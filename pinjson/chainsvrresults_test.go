...
func TestGetTxOutSetInfoResult(t *testing.T) {
	...
	tests := []struct {
		name   string
		result string
		want   pinjson.GetTxOutSetInfoResult
	}{
		{
			...
			want: pinjson.GetTxOutSetInfoResult{
				...
				DiskSize: 1,
				TotalAmount: func() pinutil.Amount {
					a, err := pinutil.NewAmount(0.2)
					if err != nil {
						panic(err)
					}
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var out pinjson.GetTxOutSetInfoResult
...
func TestChainSvrMiningInfoResults(t *testing.T) {
	...
	tests := []struct {
		name     string
		result   string
		expected pinjson.GetMiningInfoResult
	}{
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var miningInfoResult pinjson.GetMiningInfoResult
