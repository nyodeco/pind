...
func ExampleMarshalCmd() {
	...
	// optional fields.  Also, notice the call to pinjson.Bool which is a
	// convenience function for creating a pointer out of a primitive for
	// optional parameters.
	blockHash := "000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f"
	gbCmd := pinjson.NewGetBlockCmd(blockHash, pinjson.Int(0))
	...
	id := 1
	marshalledBytes, err := pinjson.MarshalCmd(pinjson.RpcVersion1, id, gbCmd)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleUnmarshalCmd() {
	...
	gbCmd, ok := cmd.(*pinjson.GetBlockCmd)
	if !ok {
...
func ExampleMarshalResponse() {
	...
	marshalledBytes, err := pinjson.MarshalResponse(pinjson.RpcVersion1, 1, 350001, nil)
	if err != nil {
		fmt.Println(err)
		return
	}
