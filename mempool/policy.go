...
const (
	...
	DefaultMinRelayTxFee = pinutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee pinutil.Amount) int64 {
	...
	if minFee < 0 || minFee > pinutil.MaxSatoshi {
		minFee = pinutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *pinutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee pinutil.Amount) bool {
...
func checkTransactionStandard(tx *pinutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee pinutil.Amount,
...
func GetTxVirtualSize(tx *pinutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
