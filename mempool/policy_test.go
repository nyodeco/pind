...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		relayFee pinutil.Amount // minimum relay transaction fee.
...
func TestCheckPkScriptStandard(t *testing.T) {
	var pubKeys [][]byte
	for i := 0; i < 4; i++ {
		pk, err := pinec.NewPrivateKey(pinec.S256())
		if err != nil {
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wire.TxOut{Value: pinutil.MaxSatoshi, PkScript: pkScript},
			pinutil.MaxSatoshi,
			false,
		},
		{
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	addr, err := pinutil.NewAddressPubKeyHash(addrHash[:],
	...
	for _, test := range tests {
		...
		err := checkTransactionStandard(pinutil.NewTx(&test.tx),
