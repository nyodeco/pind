...
import (
	...
	"github.com/nyodeco/pind/addrmgr"
	...
	"github.com/nyodeco/pind/mining/cpuminer"
	"github.com/nyodeco/pind/netsync"
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a pinutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := pinutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a pinutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := pinutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > pinutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, pinutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *pinutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			pindLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new pind server configured to listen on addr for the
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	amgr := addrmgr.New(cfg.DataDir, pindLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *pinutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
