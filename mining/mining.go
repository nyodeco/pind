...
const (
	...
	MinHighPriority = pinutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr pinutil.Address) (*pinutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *pinutil.Tx, height int32) error {
...
func logSkippedDeps(tx *pinutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress pinutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*pinutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := pinutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := pinutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A pinutil.Block should use saved in the state to avoid
