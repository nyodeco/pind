...
const (
	// svcName is the name of pind service.
	svcName = "pindsvc"
...
// logServiceStartOfDay logs information about pind when the main server has
...
// pindService houses the main service handler which handles all service
// updates and launching pindMain.
type pindService struct{}
...
// long-running pindMain (which is the real meat of pind), handles service
// change requests, and notifies the service control manager of changes.
func (s *pindService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start pindMain in a separate goroutine so the service can start
...
// installService attempts to install the pind service.  Typically this should
...
// removeService attempts to uninstall the pind service.  Typically this should
...
// startService attempts to start the pind service.
...
func serviceMain() (bool, error) {
	...
	// needed to run pind on Windows in CI environments like Travis.
	...
	err = svc.Run(svcName, &pindService{})
	if err != nil {
