...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-pind.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "pind")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of pind binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-pind.conf")
