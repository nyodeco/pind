...
var (
	...
	// mainPowLimit is the highest proof of work value a PIN block can
	// have for the main network.  It is the value 2^236 - 1.
	mainPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 236), bigOne)

	// regressionPowLimit is the highest proof of work value a PIN block
	// can have for the regression test network.  It is the value 2^255 - 1.
	regressionPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 255), bigOne)

	// testNet3PowLimit is the highest proof of work value a PIN block
	// can have for the test network (version 3).  It is the value
	// 2^224 - 1.
	testNet3PowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 236), bigOne)

	// simNetPowLimit is the highest proof of work value a PIN block
...
// Params defines a PIN network by its parameters.  These parameters may be
// used by PIN applications to differentiate networks as well as addresses
...
type Params struct {
	...
	// RetargetAdjustmentFactorUp/Down is the adjustment factor used to limit
	...
	// PowNoRetargeting specifies whether the block retargeting calculation is performed.
...
var MainNetParams = Params{
	...
	RetargetAdjustmentFactorDown: 3,                   // 25% less, 400% more
	...
	Deployments: [DefinedDeployments]ConsensusDeployment{
		...
		DeploymentCSV: {
			BitNumber:  0,
			StartTime:  1522562766,
			ExpireTime: 1554098766,
		},
		DeploymentSegwit: {
			BitNumber:  1,
			StartTime:  1522562766,
...
// PIN network.  Not to be confused with the test PIN network (version
...
var RegressionNetParams = Params{
	...
	// Human-readable part for Bech32 encoded segwit addresses, as defined in
	// BIP 173.
	Bech32HRPSegwit: "pcrt", // always bcrt for reg test net
...
// TestNet3Params defines the network parameters for the test PIN network
...
// SimNetParams defines the network parameters for the simulation test PIN
...
var SimNetParams = Params{
	...
	// Human-readable part for Bech32 encoded segwit addresses, as defined in
	// BIP 173.
	Bech32HRPSegwit: "sp", // always sp for sim net
...
var (
	// ErrDuplicateNet describes an error where the parameters for a PIN
	// network could not be set due to the network already being a standard
	// network or previously-registered into this package.
	ErrDuplicateNet = errors.New("duplicate PIN network")
...
var (
	registeredNets       = make(map[wire.PinNet]struct{})
...
// Register registers the network parameters for a PIN network.  This may
