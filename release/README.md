# `pind`'s Reproducible Build System

This package contains the build script that the `pind` project uses in order to
...
and `cmd/pinctl/version.go`.
For example (taken from [f3ec130](https://github.com/nyodeco/pind/commit/f3ec13030e4e828869954472cbc51ac36bee5c1d)):
```diff
diff --git a/cmd/pinctl/version.go b/cmd/pinctl/version.go
index 2195175c71..f65cacef7e 100644
--- a/cmd/pinctl/version.go
+++ b/cmd/pinctl/version.go
...
1. `git clone https://github.com/nyodeco/pind.git`
2. `cd pind`
3. `./release/release.sh <TAG> # <TAG> is the name of the next release/tag`

This will then create a directory of the form `pind-<TAG>` containing archives
...
Now that the directory `pind-<TAG>` is created, the manifest file needs to be
...
which roughly follow the format of [previous release notes](https://github.com/nyodeco/pind/releases/tag/v0.20.1-beta).
...
When following the GitHub documentation, include every file in the `pind-<TAG>`
directory.
...
5. Ensure `go` is installed, matching the same version as noted in the release
   notes. 
6. Obtain a copy of `pind`'s source code with `git clone
   https://github.com/nyodeco/pind` and checkout the source code of the
...
8. Extract the archive found in the `pind-<TAG>` directory created by the
   release script and recompute the `SHA256` hash of the release binaries (pind
   and pinctl) with `shasum -a 256 <filename>`. These should match __exactly__
