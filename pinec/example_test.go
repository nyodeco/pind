...
func Example_signMessage() {
	...
	privKey, pubKey := pinec.PrivKeyFromBytes(pinec.S256(), pkBytes)
...
func Example_verifySignature() {
	...
	pubKey, err := pinec.ParsePubKey(pubKeyBytes, pinec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := pinec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := pinec.PrivKeyFromBytes(pinec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := pinec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
