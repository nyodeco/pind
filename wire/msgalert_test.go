...
func TestMsgAlert(t *testing.T) {
	...
	// Test PinEncode with Payload == nil
	var buf bytes.Buffer
	err := msg.PinEncode(&buf, pver, encoding)
	if err != nil {
		t.Error(err.Error())
	}
	...
	if !bytes.Equal(buf.Bytes(), expectedBuf) {
		t.Errorf("PinEncode got: %s want: %s",
...
func TestMsgAlertWire(t *testing.T) {
	...
	for i, test := range tests {
		...
		var buf bytes.Buffer
		err := test.in.PinEncode(&buf, test.pver, test.enc)
		if err != nil {
			t.Errorf("PinEncode #%d error %v", i, err)
			continue
		}
		if !bytes.Equal(buf.Bytes(), test.buf) {
			t.Errorf("PinEncode #%d\n got: %s want: %s", i,
		...
		err = msg.PinDecode(rbuf, test.pver, test.enc)
		if err != nil {
			t.Errorf("PinDecode #%d error %v", i, err)
			continue
		}
		if !reflect.DeepEqual(&msg, test.out) {
			t.Errorf("PinDecode #%d\n got: %s want: %s", i,
...
func TestMsgAlertWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := test.in.PinEncode(w, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.writeErr) {
			t.Errorf("PinEncode #%d wrong error got: %v, want: %v",
				i, err, test.writeErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.writeErr {
				t.Errorf("PinEncode #%d wrong error got: %v, "+
		...
		err = msg.PinDecode(r, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.readErr) {
			t.Errorf("PinDecode #%d wrong error got: %v, want: %v",
				i, err, test.readErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.readErr {
				t.Errorf("PinDecode #%d wrong error got: %v, "+
	...
	w := new(bytes.Buffer)
	err := baseMsgAlert.PinEncode(w, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.PinEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
	...
	buf := *new(bytes.Buffer)
	err = baseMsgAlert.PinEncode(&buf, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.PinEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
