...
func (msg *MsgTx) PinDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count == TxFlagMarker && enc == WitnessEncoding {
		...
		if flag[0] != WitnessFlag {
			...
			return messageError("MsgTx.PinDecode", str)
		}
...
// the Version field in the transaction.  This function differs from PinDecode
// in that PinDecode decodes from the bitcoin wire protocol as it was sent
...
// the transaction.  This function differs from PinEncode in that PinEncode
