...
func TestVersionWireErrors(t *testing.T) {
	...
	// Ensure calling MsgVersion.PinDecode with a non *bytes.Buffer returns
	// error.
	fr := newFixedReader(0, []byte{})
	if err := baseVersion.PinDecode(fr, pver, enc); err == nil {
		t.Errorf("Did not received error when calling " +
			"MsgVersion.PinDecode with non *bytes.Buffer")
	}
	...
	for i, test := range tests {
		...
		err = msg.PinDecode(buf, test.pver, test.enc)
...
func TestVersionOptionalFields(t *testing.T) {
	...
	uaVersion.UserAgent = "/pindtest:0.0.1/"
