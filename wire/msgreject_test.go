...
func TestRejectLatest(t *testing.T) {
	...
	err := msg.PinEncode(&buf, pver, enc)
	if err != nil {
...
func TestRejectCrossProtocol(t *testing.T) {
	...
	var buf bytes.Buffer
	err := msg.PinEncode(&buf, ProtocolVersion, BaseEncoding)
	if err != nil {
		t.Errorf("encode of MsgReject failed %v err <%v>", msg, err)
	}
	...
	readMsg := MsgReject{}
	err = readMsg.PinDecode(&buf, RejectVersion-1, BaseEncoding)
	if err == nil {
		t.Errorf("encode of MsgReject succeeded when it shouldn't "+
			"have %v", msg)
	}
...
func TestRejectWire(t *testing.T) {
	...
	for i, test := range tests {
		...
		var buf bytes.Buffer
		err := test.msg.PinEncode(&buf, test.pver, test.enc)
		if err != nil {
