...
func commandUsage(method string) {
	usage, err := pinjson.MethodUsageText(method)
	if err != nil {
...
func main() {
	...
	method := args[0]
	usageFlags, err := pinjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := pinjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// pinjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(pinjson.Error); ok {
		...
		// The error is not a pinjson.Error and this really should not
	...
	marshalledJSON, err := pinjson.MarshalCmd(pinjson.RpcVersion1, 1, cmd)
	if err != nil {
