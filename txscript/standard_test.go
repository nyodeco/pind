...
// newAddressPubKey returns a new pinutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) pinutil.Address {
	addr, err := pinutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new pinutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) pinutil.Address {
	addr, err := pinutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new pinutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) pinutil.Address {
	addr, err := pinutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the pinutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the pinutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := pinutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := pinutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := pinutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := pinutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := pinutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*pinutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*pinutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*pinutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*pinutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*pinutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
