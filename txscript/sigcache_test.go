...
// test data.
func genRandomSig() (*chainhash.Hash, *pinec.Signature, *pinec.PublicKey, error) {
	privKey, err := pinec.NewPrivateKey(pinec.S256())
	if err != nil {
...
func TestSigCacheAddExists(t *testing.T) {
	...
	sig1Copy, _ := pinec.ParseSignature(sig1.Serialize(), pinec.S256())
	key1Copy, _ := pinec.ParsePubKey(key1.SerializeCompressed(), pinec.S256())
...
func TestSigCacheAddEvictEntry(t *testing.T) {
	...
	for i := uint(0); i < sigCacheSize; i++ {
		...
		sigCopy, _ := pinec.ParseSignature(sig.Serialize(), pinec.S256())
		keyCopy, _ := pinec.ParsePubKey(key.SerializeCompressed(), pinec.S256())
	...
	sigNewCopy, _ := pinec.ParseSignature(sigNew.Serialize(), pinec.S256())
	keyNewCopy, _ := pinec.ParsePubKey(keyNew.SerializeCompressed(), pinec.S256())
