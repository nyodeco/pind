...
func RawTxInWitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	...
	key *pinec.PrivateKey) ([]byte, error) {
...
func WitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int, amt int64,
	subscript []byte, hashType SigHashType, privKey *pinec.PrivateKey,
	...
	pk := (*pinec.PublicKey)(&privKey.PublicKey)
	var pkData []byte
	if compress {
...
func RawTxInSignature(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, key *pinec.PrivateKey) ([]byte, error) {
...
func SignatureScript(tx *wire.MsgTx, idx int, subscript []byte, hashType SigHashType, privKey *pinec.PrivateKey, compress bool) ([]byte, error) {
...
func p2pkSignatureScript(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *pinec.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey)
	if err != nil {
		return nil, err
	}
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []pinutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []pinutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []pinutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []pinutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		pSig, err := pinec.ParseDERSignature(tSig, pinec.S256())
		if err != nil {
			continue
		}
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*pinutil.AddressPubKey)
...
type KeyDB interface {
	GetKey(pinutil.Address) (*pinec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(pinutil.Address) (*pinec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address pinutil.Address) (*pinec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(pinutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(pinutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address pinutil.Address) ([]byte, error) {
	return sc(address)
}
