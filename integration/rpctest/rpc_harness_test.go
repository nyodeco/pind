...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt pinutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(pinutil.Amount(5 * pinutil.SatoshiPerBitcoin))
	...
	txid = genSpend(pinutil.Amount(500 * pinutil.SatoshiPerBitcoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(pinutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*pinutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, pinutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := pinutil.Amount(250 * pinutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = pinutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := pinutil.Amount(50 * pinutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := pinutil.Amount(numMatureOutputs * 50 * pinutil.SatoshiPerBitcoin)
