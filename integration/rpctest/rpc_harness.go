...
// Harness fully encapsulates an active pind process to provide a unified
// platform for creating rpc driven integration tests involving pind. The
// active pind node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active pind process is fully
...
// used. If a custom pind executable is specified, it will be used to start the
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*pinutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the pind node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []pinutil.Address{h.wallet.coinbaseAddr}
	if err := h.Client.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure pind properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created pind
...
func (h *Harness) NewAddress() (pinutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() pinutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate pinutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate pinutil.Amount, change bool) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*pinutil.Tx, blockVersion int32,
	blockTime time.Time) (*pinutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*pinutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*pinutil.Block, error) {
	...
	prevBlock := pinutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "pind", "rpctest")
