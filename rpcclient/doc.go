...
Bitcoin RPC server that uses a pind/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with pind (https://github.com/nyodeco/pind),
pinwallet (https://github.com/btcsuite/pinwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, pind and
pinwallet provide a websocket interface that is more efficient than the standard
...
pind or pinwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by pind and
pinwallet only uses a single connection that remains open and allows
...
All notifications provided by pind require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by pind (and
pinwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that pind intentionally separates the wallet
functionality into a separate process named pinwallet.  This means if you are
connected to the pind RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, pinwallet provides pass through treatment
...
detected by type asserting the error in a *pinjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*pinjson.RPCError); ok {
  		switch jerr.Code {
  		case pinjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - pindwebsockets
   ...
   block count
 - btcwalletwebsockets
   Connects to a pinwallet RPC server using TLS-secured websockets, registers
